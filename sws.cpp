...
void WebSocket::send(const std::string& message) {
    sendData(TEXT_FRAME, (uint64_t)message.size(), message.begin(), message.end());
}

void WebSocket::sendBinary(const std::string& message) {
    sendData(BINARY_FRAME, (uint64_t)message.size(), message.begin(), message.end());
}
...
void WebSocket::sendPing() {
    std::string empty;
    sendData(PING, (uint64_t)empty.size(), empty.begin(), empty.end());
}
...
void WebSocket::dispatchInternal(CallbackImp& callable) {
    ...
    while (true) {
        ...
        if (ws.opcode == TEXT_FRAME || ws.opcode == BINARY_FRAME || ws.opcode == CONTINUATION) {
            ...
            OpCodeType opcode = CONTINUATION;

            m_mtx_rxbuf.lock();
            if (ws.mask) { for (size_t i = 0; i != ws.N; ++i) { rxbuf[i + ws.header_size] ^= ws.masking_key[i & 0x3]; } }

            if (ws.opcode == TEXT_FRAME || (ws.opcode == CONTINUATION && last_opcode == TEXT_FRAME)) {
                opcode = TEXT_FRAME;
                recved_frame.insert(recved_frame.end(), rxbuf.begin() + ws.header_size, rxbuf.begin() + ws.header_size + (size_t)ws.N);// just feed
            }
            else if (ws.opcode == BINARY_FRAME || (ws.opcode == CONTINUATION && last_opcode == BINARY_FRAME)) {
                opcode = BINARY_FRAME;
                recved_frame.insert(recved_frame.end(), rxbuf.begin() + ws.header_size, rxbuf.begin() + ws.header_size + (size_t)ws.N);// just feed
            }
...
void WebSocketClient::Run() {
    while (m_running && m_ws && m_ws->getReadyState() != WebSocket::CLOSED) {
        m_ws->poll();
